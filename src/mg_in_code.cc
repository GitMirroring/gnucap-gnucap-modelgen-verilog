...
void Variable_Decl::parse(CS& f)
{
  ...
  if(attr.has_attributes(tag_t(&f))) {
    ...
    if(  a->operator[](std::string("_state")) != "0"
      || a->operator[](std::string("desc")) != "0"
      || a->operator[](std::string("units")) != "0" ){
      _stt.override_state();
      if(a->operator[](std::string("_nostate")) != "0" ){ untested();
	incomplete(); // conflicting request.
      }else if(a->operator[](std::string("_common")) != "0" ){ untested();
	incomplete(); // conflicting request.
      }else{
      }
    }else if(a->operator[](std::string("_common")) != "0" ){ untested();
      _stt.override_set();
    }else if(a->operator[](std::string("_nostate")) != "0" ){ untested();
      _stt.override_tmp();
    }else{
    }
  }else{
